...
const TEXT_EXT = new Set([".js", ".mjs", ".ts", ".tsx", ".json", ".md", ".html", ".css", ".yml", ".yaml"]);
...
if (!process.env[EXPERIENTIAL_CONFIG.envKeyName]?.trim()) {
  console.error(`${C.red}Thiếu ${EXPERIENTIAL_CONFIG.envKeyName}. Export key hoặc thêm EXPLABS_API_KEY=xpl_... vào .env.${C.reset}`);
  process.exit(1);
}

function safePath(path) {
  const full = resolve(ROOT, String(path || ""));
  if (full !== ROOT && !full.startsWith(ROOT + sep)) throw new Error(`Path vượt project: ${path}`);
  return full;
}
function ext(path) {
...
function local(command, timeout = 60_000) {
  try {
    return { ok: true, text: execSync(command, { cwd: ROOT, encoding: "utf8", timeout, stdio: ["ignore", "pipe", "pipe"] }) };
  } catch (error) {
...
function commandResult(result) {
  ...
  const body = lines.length <= 44 ? lines : [...lines.slice(0, 20), `… ${lines.length - 40} lines omitted …`, ...lines.slice(-20)];
...
function profile(task) {
  ...
  if (["architecture", "kiến trúc", "deterministic", "tất định", "systemic", "rewrite"].some((x) => text.includes(x))) return "deep";
  if (["physics", "vật lý", "collision", "va chạm", "traffic", "gameplay", "playcanvas", "three.js", "render", "đồ họa", "đồ hoạ", "visual", "camera", "performance", "tối ưu"].some((x) => text.includes(x))) return "complex";
  if (["typo", "format", "prettier", "readme", "docs", "copy"].some((x) => text.includes(x))) return "small";
  return "normal";
}
...
function reasoning(session, step) {
  ...
  if (session.lastTools.length && session.lastTools.every((x) => ["git_diff", "run_command"].includes(x))) return "medium";
  return "high";
}
...
function usage(session, response) {
  ...
  session.usage.cached += u.prompt_tokens_details?.cached_tokens ?? u.input_tokens_details?.cached_tokens ?? 0;
...
function searchCode(query, directory = "src", limit = 24) {
  ...
  for (const file of walk(directory)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (hits.length >= Math.min(40, Math.max(1, Number(limit) || 24))) return hits;
    }
  }
  return hits;
}
function patchFiles(patch) {
  return [...patch.matchAll(/^\+\+\+ b\/(.+)$/gm)].map((match) => match[1].trim());
}
...
const TOOLS = [
  {
    ...
    function: {
      name: "search_code",
      description: "Tìm symbol/chuỗi và trả file:line. Luôn dùng trước khi đọc source nếu chưa biết vị trí.",
      parameters: { type: "object", properties: { query: { type: "string" }, directory: { type: "string" }, max_results: { type: "integer" } }, required: ["query"] },
    },
  },
  {
    type: "function",
    function: {
      ...
      description: "Đọc đúng range source có line numbers; hard cap 180 dòng/lần.",
      parameters: { type: "object", properties: { path: { type: "string" }, start_line: { type: "integer" }, end_line: { type: "integer" } }, required: ["path", "start_line", "end_line"] },
    },
  },
  {
    type: "function",
    function: {
      name: "edit_file",
      description: "Exact replacement cho sửa nhỏ; ưu tiên hơn xuất lại nguyên file.",
  ...
  },
  {
    ...
    function: {
      name: "apply_patch",
      description: "Áp unified git patch. Ưu tiên cho thay đổi nhiều đoạn/nhiều file để giảm output token.",
  ...
  },
  {
    ...
    function: {
      name: "run_command",
      description: "Chạy validation allowlist: tests/typecheck/quality/format/smoke hoặc git status/diff/log. Không dùng để đọc source.",
...
function executeTool(session, name, args) {
  try {
    if (session.tools >= session.budget.tools) throw new Error("tool budget exhausted");
    session.tools++;
    if (name === "search_code") {
      const hits = searchCode(args.query, args.directory || "src", args.max_results || 24);
      return { ok: true, content: hits.join("\n") || "NO MATCHES", summary: `search '${args.query}': ${hits.length} hit(s)` };
    }
    if (name === "read_file_range") {
      const path = String(args.path || "");
      if (!session.files.has(path) && session.files.size >= session.budget.files) throw new Error("file read budget exhausted");
      const lines = readFileSync(safePath(path), "utf8").split("\n");
      const start = Math.max(1, Math.min(lines.length, Number(args.start_line) || 1));
      const remaining = session.budget.lines - session.lines;
      if (remaining <= 0) throw new Error("line read budget exhausted");
      const end = Math.min(lines.length, Number(args.end_line) || start, start + MAX_RANGE - 1, start + remaining - 1);
      const part = lines.slice(start - 1, end);
      session.files.add(path);
      session.lines += part.length;
      return { ok: true, content: `FILE ${path} ${start}-${end}/${lines.length}\n${part.map((line, i) => `${start + i}| ${line}`).join("\n")}`, summary: `read ${path}:${start}-${end}` };
    }
    if (name === "edit_file") {
      const path = String(args.path || "");
      ...
      if (!oldText || oldText.length + newText.length > MAX_PATCH) throw new Error("edit too large/empty");
      const first = before.indexOf(oldText);
      if (first < 0 || before.indexOf(oldText, first + oldText.length) >= 0) throw new Error("old_string missing or not unique");
      writeFileSync(full, before.replace(oldText, newText), "utf8");
      session.changed.add(path);
      return { ok: true, content: `SUCCESS edited ${path}`, summary: `edited ${path}` };
    }
    if (name === "apply_patch") {
      const patch = String(args.patch || "");
      if (!patch.trim() || patch.length > MAX_PATCH) throw new Error(`patch must be 1..${MAX_PATCH} chars`);
      const files = patchFiles(patch);
      if (!files.length) throw new Error("patch needs +++ b/<path>");
      for (const file of files) safePath(file);
      execFileSync("git", ["apply", "--check", "--whitespace=nowarn", "-"], { cwd: ROOT, input: patch, encoding: "utf8", timeout: 15_000 });
      execFileSync("git", ["apply", "--whitespace=nowarn", "-"], { cwd: ROOT, input: patch, encoding: "utf8", timeout: 15_000 });
      files.forEach((file) => session.changed.add(file));
      return { ok: true, content: `SUCCESS patched ${files.join(", ")}`, summary: `patched ${files.join(", ")}` };
    }
    if (name === "git_diff") {
      const paths = Array.isArray(args.paths) ? args.paths.slice(0, 8) : [];
      paths.forEach(safePath);
      const text = execFileSync("git", ["diff", "--unified=3", ...(paths.length ? ["--", ...paths] : [])], { cwd: ROOT, encoding: "utf8", timeout: 15_000 });
      return { ok: true, content: compact(text || "NO DIFF", 10_000), summary: `reviewed diff${paths.length ? `: ${paths.join(", ")}` : ""}` };
    }
    if (name === "run_command") {
      const command = String(args.command || "").trim();
      if (!allowedCommand(command)) throw new Error("command outside validation allowlist");
      const result = local(command, command.includes("smoke") ? 120_000 : 60_000);
      if (!result.ok) session.failures++;
      if (command === "npm run quality" && result.ok) session.quality = true;
      return { ok: result.ok, content: commandResult(result), summary: `${command}: ${result.ok ? "PASS" : "FAIL"}` };
    }
    throw new Error(`unknown tool ${name}`);
  } catch (error) {
    session.failures++;
    return { ok: false, content: `ERROR ${name}: ${error.message || error}`, summary: `${name} failed: ${error.message || error}` };
  }
}
...
function messagesFor(task, session, repoSnapshot, pending) {
  const state = compact(session.state.map((x) => `- ${x}`).join("\n") || "- none", 6_000);
  ...
  const messages = [{ role: "system", content: `${SYSTEM}\n\n${dynamic}` }, { role: "user", content: task }];
  if (pending) messages.push(pending.assistant, ...pending.tools);
  return messages;
}
function finalGate(session) {
  if (!session.changed.size || session.quality || session.gateTried) return false;
  ...
  remember(session, `automatic npm run quality: FAIL ${compact(commandResult(result), 1_700)}`);
...
async function runTask(task, requestedBudget) {
  ...
  console.log(`${C.dim}Budget: ${session.budget.model} model · ${session.budget.tools} tools · ${session.budget.lines} lines · ${session.budget.files} files${C.reset}\n`);
  ...
  while (session.calls < session.budget.model) {
    ...
    console.log(`${C.dim}--- call ${session.calls}/${session.budget.model} · reasoning=${effort} · ~${Math.ceil(JSON.stringify(messages).length / 4).toLocaleString()} input tokens before tool schema ---${C.reset}`);
    let response;
    try {
    ...
    if (!message) break;
    if (message.content?.trim()) console.log(`${C.green}Astra:${C.reset} ${compact(message.content, 1_500)}\n`);
    ...
    for (const call of calls) {
      ...
      console.log(`${C.yellow}⚡ ${call.function.name}${C.reset} ${C.dim}${compact(JSON.stringify(args), 160)}${C.reset}`);
      ...
      toolMessages.push({ role: "tool", tool_call_id: call.id, content: compact(result.content) });
    }
    pending = {
      assistant: { role: "assistant", content: message.content ? compact(message.content, 800) : null, tool_calls: message.tool_calls },
      tools: toolMessages,
    };
  }
  ...
  if (!finalText) {
    ...
    finalText = needsRepair ? "Hết model budget khi quality còn fail." : "Dừng tại budget hiện tại; xem summary.";
  }
  const u = session.usage;
  console.log(`\n${C.cyan}${C.bold}🏁 ${finalText}${C.reset}`);
  console.log(`${C.dim}Usage: ${u.requests} requests · input ${u.input.toLocaleString()} · cached ${u.cached.toLocaleString()} · output ${u.output.toLocaleString()} · total ${u.total.toLocaleString()} tokens${C.reset}`);
  console.log(`${C.dim}Local: ${session.tools} tools · ${session.lines} lines/${session.files.size} files · changed ${[...session.changed].join(", ") || "none"} · quality ${session.quality ? "PASS" : "pending"}${C.reset}\n`);
}
...
function parseArgs(argv) {
  ...
  if (budget && !BUDGETS[budget]) throw new Error("--budget phải là small|normal|large");
...
console.log(`${C.dim}Local tools làm việc cơ học; Astra chỉ dùng token cho quyết định/code/review.${C.reset}`);
console.log(`CLI có ${C.yellow}--budget=small|normal|large${C.reset}. Gõ exit để thoát.\n`);
const rl = readline.createInterface({ input, output });
while (true) {
  const task = (await rl.question(`${C.cyan}${C.bold}astra> ${C.reset}`)).trim();
  if (!task) continue;
